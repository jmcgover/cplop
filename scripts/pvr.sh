...
for s in "${speciesArray[@]}"; do
   plotCommand=${plotCommand}'"'$file'" using '${xCol}':($2=='$alpha' && strcol(3) eq "'${s}'" && strcol(4) eq "16-23" ?$'${yCol}':NaN):1 with labels offset 1 notitle "'${s}': 16S-23S",'
   plotCommand=${plotCommand}'"'$file'" using '${xCol}':($2=='$alpha' && strcol(3) eq "'${s}'" && strcol(4) eq "16-23" ?$'${yCol}':NaN) title "'${s}': 16S-23S",'

   plotCommand=${plotCommand}'"'$file'" using '${xCol}':($2=='$alpha' && strcol(3) eq "'${s}'" && strcol(4) eq "23-5" ?$'${yCol}':NaN):1 with labels offset 1 notitle "'${s}': 23S-5S",'
